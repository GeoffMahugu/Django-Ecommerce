...
class Product(models.Model):
    ...
    default_category = models.ForeignKey('Category', related_name='default_category', null=True, blank=True, on_delete = models.CASCADE)
...
#def image_upload_to_vari(instance, filename):
#	title = instance.product.title
#	slug = slugify(title)
#	basename, file_extension = filename.split(".")
#    
##	new_filename = "%s.%s" %(slug, file_extension)
##    new_filename = "%s.%s" %(slug, filename)
#	new_filename = "%s.%s" %(slug, filename)
#	return "products/%s" %(slug)
...
class Variation(models.Model):
    ...
    currency = models.CharField(max_length=120, null=True, blank = True, default = 'Ksh')
...
#    
#    def get_absolute_url(self):
#        return reverse('Products:SingleProductFeature', kwargs={'pk':self.pk})
