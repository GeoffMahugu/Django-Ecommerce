...
def SingleProduct(request, pk=None):
    ...
    reviews = Review.objects.filter(product = objects).filter(active = True ).order_by('-pk')
...
#            message.error(request, 'Unsuccessfull Sending Review')
    else:
...
#    return render(request, 'scategory.html',context)
    return render(request, 'base.html',context)
